...
export async function getCurrentDesktop(): Promise<Desktop | null> {
...
export async function getVirtualDesktops(
  ...
  try {
    ...
    const desktops = JSON.parse(stdout) as RawDesktop[];
...
export async function switchToDesktop(
  ...
  try {
    ...
    const desktops = preloadedDesktops ?? await getVirtualDesktops();
    ...
    if (!targetDesktop) {
      ...
      targetDesktop = desktops[0];
    }

    console.log(
      `[switchToDesktop] Switching to desktop: ${targetDesktop.name}`,
    );

    const switchStartTime = performance.now();
    try {
      await execViaPersistentPwsh(`Switch-Desktop '${targetDesktop.name}'`);
      ...
      onFailure(
        `Failed to switch to desktop '${targetDesktop.name}': ${errorMsg}`,
      );
      return;
    }
...
async function execViaPersistentPwsh(command: string): Promise<string> {
  ...
  if (!persistentPwsh?.stdin || !persistentPwsh.stdout) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function onData(data: Buffer): void {
      ...
      if (output.includes(marker)) {
        cleanup();
        const result = output.split(marker)[0].trim();
        resolve(result);
      }
    }
    ...
    function cleanup(): void {
      persistentPwsh?.stdout?.off('data', onData);
      persistentPwsh?.stderr?.off('data', onError);
      persistentPwsh?.off('close', onClose);
    }

    persistentPwsh.stdout.on('data', onData);
    persistentPwsh.stderr.on('data', onError);
    persistentPwsh.on('close', onClose);

    persistentPwsh.stdin.write(`${wrappedCommand}\n`);
  });
}
